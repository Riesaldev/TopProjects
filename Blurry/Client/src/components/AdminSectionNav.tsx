...
function toLabel(segment: string) {
  return ADMIN_LABELS[segment] ?? segment.replace(/-/g, " ").replace(/\b\w/g, (char) => char.toUpperCase());
}

function buildAdminCrumbs(pathname: string) {
  const segments = pathname.split("/").filter(Boolean);
  const crumbs: Array<{ label: string; href?: string }> = [{ label: "Admin", href: "/admin/dashboard" }];
    ...
    <header className="sticky top-0 z-40 w-full border-b border-zinc-800/70 bg-zinc-950/85 backdrop-blur-md">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-3 py-3 sm:px-4">
        <div className="min-w-0">
          <nav aria-label="Breadcrumb de administrador" className="mb-1 overflow-x-auto">
